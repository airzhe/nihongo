...
const VocabularyApp = {
    ...
    bindEvents() {
        ...
        this.elements.quizContainer.addEventListener('click', async (e) => { 
            ...
            if (optionBtn) { // Allow clicks even when input is locked for review purposes
                const hasBeenAnswered = this.state.answerLog.some(log => log.questionIndex === this.state.currentQuestion);

                if (hasBeenAnswered) {
                    // Review mode: play audio, potentially add to wrong words, and navigate
                    ...
                    if (question && question.options[selectedIndex]) {
                        // Manually add to wrong words list if a wrong option is selected during review
                        if (!question.options[selectedIndex].correct) {
                            WrongWordsManager.addOrUpdate(question.vocab, 'manual_review_add');
                        }
                        if (question.options[selectedIndex].vocab) {
                            this.playWordAudio(question.options[selectedIndex].vocab);
                        }
                    }
                    setTimeout(() => this.navigateToNextQuestion(), 150);
                } else if (!this.state.isInputLocked) {
            ...
            if (detailedItem) {
                const word = detailedItem.dataset.w;
                const question = this.state.currentQuiz[this.state.currentQuestion];
                const option = question.options.find(opt => opt.vocab.w === word);
                if (option && option.vocab) {
                    this.playWordAudio(option.vocab);
                }
                return;
            }
    ...
    // --- MODIFIED: Helper function to play word audio ---
    playWordAudio(vocab) {
        if (!vocab || !vocab.w || !vocab.lesson) return;
        ...
        // New logic: data/audio/n1/lesson1_生.mp3 -> data/audio/n1/1_生.mp3
        const lessonNumber = parseInt(String(vocab.lesson).replace('lesson', ''), 10);
        const audioPath = `data/audio/${currentLevel}/${lessonNumber}_${vocab.w}.mp3`;
        
        audioPlayer.src = audioPath;
        audioPlayer.play().catch(error => {
            console.warn(`Could not play audio for "${vocab.w}" from lesson "${vocab.lesson}". Path: ${audioPath}. File might be missing.`, error);
        });
    },
    ...
    submitAnswer(selectedIndex) {
        ...
        // Do not play audio when submitting an answer in review mode.
        // Audio is handled by the click/keydown handlers in review mode.
        if (selectedOption && selectedOption.vocab && !this.state.answerLog.some(log => log.questionIndex === this.state.currentQuestion)) {
            this.playWordAudio(selectedOption.vocab);
        }
    ...
    startQuiz() { 
        ...
        let sourceVocab = selectedLessons.includes('all') 
            ? Object.entries(this.state.allVocabulary).flatMap(([lesson, words]) => words.map(word => ({ ...word, lesson })))
            : selectedLessons.flatMap(lesson => (this.state.allVocabulary[lesson] || []).map(word => ({ ...word, lesson }))); 
    ...
    generateQuestions(vocabList, mode) {
        const wrongOptionPool = this.shuffleArray(
            Object.entries(this.state.allVocabulary).flatMap(([lesson, words]) =>
                words.map(word => ({ ...word, lesson }))
            )
        );
    ...
    createQuizHTML(question, historyEntry, isReviewMode = false) {
        ...
        const optionsHTML = question.options.map((option, index) => { 
            ...
            return `<button class="${baseClasses} ${stateClasses}" data-index="${index}" data-word="${option.vocab.w}"><span class="${numberClasses}">${index + 1}.</span><span class="${textClasses}">${option.text}</span></button>`;
        }).join(''); 
        ...
        if (e.key === ' ') {
            ...
            if (hasBeenAnswered) {
                ...
                if (question && question.options[selectedIndex]) {
                    ...
                    // Only play audio for the correct answer during review
                    if (question.options[selectedIndex].vocab && question.options[selectedIndex].correct) {
                        this.playWordAudio(question.options[selectedIndex].vocab);
                    }
                }
